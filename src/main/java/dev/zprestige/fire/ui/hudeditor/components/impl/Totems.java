...
public class Totems extends HudComponent {
    ...
    public void render() {
        ...
        if (totems > 0) {
            ...
            Main.fontManager.drawStringWithShadow(totemString, new Vector2D(((position.getX()) + (11 - (Main.fontManager.getStringWidth(totemString) / 2f))) / 0.8f, position.getY() / 0.8f), new Color(red(totems) / 255.0f, green(totems) / 255.0f, 0.0f).getRGB());
